...
- `docs/archive/AGENTS-AUDIT-2025.md` – archived SolSeeker-era backlog (for historical reference only)
- `OPERATIONS.md` – operational runbook for deployments
- `web/public/dashboard_api_audit.md` – frontend ↔ API mapping

## Codex Agent - Network Wiring + Bootstrap UX

**Date:** 2026-02-05

### Summary
- Realigned RPC calls to The Block primitives (`balance`, `net.overlay_status`, `net.peer_stats_all`) so wallet checks, peer counts, and network strength populate instead of returning zero/defaults; governor economics now feed market utilization/margin in the dashboard snapshot.
- Fixed the custom WebSocket server NameError (`data` undefined) and enabled socket reuse in the HTTP server to prevent “port already in use” crashes during restart.
- Landing page now uses the richer SolSeeker-style chrome, defaults to network health when gates are closed or genesis isn’t ready, and ensures a local wallet is provisioned before redirecting. Static → API fallback now points at :5000 to match `just start-node`.

### Next Steps
- Surface bootstrap/genesis and wallet funding state in the dashboard hero so operators immediately see whether trading is blocked on funding vs. gate readiness.
- Consider persisting the chosen API port (if non-default) to `localStorage` during `start_node.sh` to avoid mismatched ports when the dashboard auto-shifts off :5000.

## Codex Agent - Dashboard UX 1% Polish

**Date:** 2026-02-04

### Summary
- Made the dashboard hero the single source for uptime, block height, engine, and license; added inline “last updated” chips to every data panel and gray-out controls while data is stale.
- Gated markets/charts/receipts behind real responses: panels stay hidden until `/theblock/dashboard` or the websocket replies and collapse to one “coming soon” card on failure; offline banner now shows retry counts plus a latency sparkline fed by `/health`.
- Standardized navigation and visual rhythm (pills/padding/shadows/focus rings) across dashboard, settings, whales, sentiment, and strategies; side drawer now closes on route change and highlights the active cluster.
- Hardened settings with inline validation for API base/key/trading mode/assets (save disabled until valid), cached asset + feature schema lists, and local toasts/status pills for saves.

### Next Steps
- Monitor `/whales`, `/sentiment/*`, `/strategy/*`, and `/backtest` traffic now that they are live; keep dashboards aligned when schemas evolve.
- Keep `/health` available to the frontend so the latency sparkline and offline banner stay accurate; align dashboard_api_audit.md when new endpoints land.
- Track client stale/fresh flip telemetry (`block_buster_observe`) now emitted on dashboard/whales/sentiment/strategies/settings; keep flag documented per §0.3.

## Codex Agent - Whale/Sentiment/Strategy Wiring

**Date:** 2026-02-04

### Summary
- Added first-party endpoints in `src/the_block/dashboard_server.py` for `/whales` (+ WS), `/sentiment/{trending,influencers,pulse}`, `/strategy/{performance_matrix,breakdown,matrix}`, and `/backtest` (+ WS progress).
- Dashboard server now emits whale, sentiment, and strategy snapshots derived from live receipts/markets; backtests stream deterministic progress/equity curves without third-party deps.
- Frontend pages (`whales.html`, `sentiment.html`, `strategies.html`) render live data (tables + accordions) instead of “coming soon” gates, and `dashboard_api_audit.md` documents the wired surfaces.

## Codex Agent - Dashboard UX Layout Refresh

**Date:** 2026-02-04

### Summary
- Reworked `web/public/dashboard.html` header + hero strip to surface uptime, block height, engine state, and license status as first-glance pills.
- Regrouped side navigation into Operate / Trade / Analyze / Configure clusters with deep links to strategies, whales, sentiment, MEV, and settings.
- Added responsive safeguards: market grid collapses to single column on small screens and recent receipts now render both a table (desktop) and accordion cards (mobile) populated from the same data.
- Introduced shared Chart.js theming and chart state helpers (`web/public/js/charting.js`), wired dashboard charts to the helper, and added empty/error fallbacks without pulling new dependencies.
- Vendored frontend assets (tailwind + chart renderer) under `web/public/{css,js/vendor}` to eliminate CDN fetches; added dashboard timestamp badges and market fallback panel per `dashboard_api_audit.md` guidance.

### Next Steps
- Wire receipts data and uptime/engine fields to the live backend payloads; hide or debounce cards when responses are absent.
- Apply the shared pill/tile styling to remaining status widgets to keep the visual hierarchy consistent.

## Codex Agent - Next 1% First-Party Hardening

**Date:** 2026-02-03

### Summary
- Captured the “Next 1% Improvements” from `docs/apis_and_tooling.md` into the spec so the block-buster ↔ blockchain integration stays first-party only.
- Committed to connection pooling/backoff + latency tracing in `core/http_client`, periodic WAL `fsync` and compaction hooks in `core.database.SimpleDb`, and a `/health` probe that surfaces uptime, SimpleDb metrics, feature lag, and recent error counts.
- Documented the new metrics scrape endpoint (text format) with auth toggle, feature snapshot WS push channel, and DAL cache invalidation hook for governor-driven asset updates.

### Decisions
- Observability must route through `block_buster.utils.metrics`; scrape exposure is via the custom HTTP server, not third-party exporters.
- Feature engine snapshots now enforce invariants (length == TOTAL_DIM, no NaNs) before emit; downstream readers rely on this contract.
- Posterior model save files carry `{version, checksum}` to prevent stale loads; learning rate and weight updates are clamped to avoid runaway tuning in live mode.

### Next Steps
- Run `python -m unittest discover block-buster/tests` after each change set.
- Keep any new TODOs mirrored into §15 with file/line pointers instead of inline comments.

## Codex Agent - SimpleDb Persistence Migration

**Date:** 2026-02-03

### Summary
- Replaced the SQLModel/SQLite DAL with the first-party `SimpleDb` storage layer to comply with the zero third-party dependency contract.
- Added schema guard via `SCHEMA_HASH`, ordered key encoding, and WAL-backed column families for orders, positions, assets, prices, and meta data.
- Updated tests to drop `sqlmodel` usage; documentation now reflects the in-house database.

### Decisions
- Orders now use lexicographically sortable keys (`{order_id:020d}`) stored as JSON blobs in the `orders` column family.
- Positions persist protobuf bytes directly; assets and prices store compact JSON.
- Meta namespace keeps schema hash and order sequence counters; WAL flush on meta writes preserves crash safety.

### Next Steps
- Run targeted persistence tests after integrating with any remaining HTTP/router refactors.
- Mirror any new TODOs into §15 once additional persistence features land.

## Codex Agent - Third-Party Purge (numpy/ntplib/httpx)

**Date:** 2026-02-03

### Summary
- Replaced remaining third-party dependencies with first-party shims: in-house `np` module (pure stdlib) now backs all feature/risk/posterior math, and `ntp_client` replaces `ntplib`.
- Removed legacy `rpc_client_old.py` (requests), dropped constraints file, and guarded/updated tests that referenced external clients (`httpx`, `fastapi`, `jwt`).
- Prometheus usage swapped to `utils.metrics`; feature engines and TheBlock adapters now depend solely on first-party utilities.

### Decisions
- Added `block_buster.utils.np` (NDArray + RNG + save/load) as the canonical numpy replacement; all runtime imports now point to it.
- Metrics bindings use `utils.metrics.{Counter,Gauge,Histogram}` to avoid `prometheus_client`.
- Tests that require third-party stacks now auto-skip when those modules are absent, keeping the suite runnable in zero-dependency mode.

### Next Steps
- Run the feature/risk/posterior test subset in a clean environment to validate the numpy shim.
- Delete or refactor any remaining FastAPI-specific tests once the custom HTTP server reaches parity.

## Codex Agent - First-Party Test & Tooling Reset

**Date:** 2026-02-03

### Summary
- Replaced pytest/third-party-heavy test suite with stdlib `unittest` cases covering `np`, `metrics`, and `syschecks`.
- Removed all Python dev/test dependencies from `requirements.txt`; constraints file deleted earlier.

### Next Steps
- Wire CI to run `python -m unittest discover block-buster/tests`.
