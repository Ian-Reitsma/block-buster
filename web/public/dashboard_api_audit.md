...
## 0.a Frontend gating status (2026-02-04)

- Dashboard hero is now the single source of truth for uptime/block height/engine/license. Every data panel carries an inline “last updated” chip and stays hidden until `/theblock/dashboard` or the websocket returns; a compact offline banner shows retry counts plus a `/health`-fed latency sparkline. See `web/public/dashboard.html`.
- Markets, charts, and receipts gate behind live responses and collapse to a single “coming soon” card if their backing endpoints fail; accordions back wide tables on mobile.
- Whale, Sentiment, Strategy, and Backtest flows are now wired end-to-end: `/whales` + `/whales/ws`, `/sentiment/trending|influencers|pulse`, `/strategy/performance_matrix|breakdown|matrix`, and `/backtest` + `/backtest/ws/{id}` are served by `dashboard_server.py` and rendered by `web/public/{whales.html,sentiment.html,strategies.html}`.
...
`web/public/js/utils.js` exports helpers `formatBlock(amount)`, `formatBlockChange(amount)`, and `formatPercent(value)` to render BLOCK amounts alongside their USD equivalents and percentage deltas. All helpers gracefully handle negative values. Use them as:

```js
import { formatBlock, formatBlockChange } from './js/utils.js';
el.textContent = formatBlock(balanceBlock);
```
