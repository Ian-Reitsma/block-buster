...
describe('BlockWebSocket', () => {
  ...
  it('should send JSON messages', () => {
        const message = { type: "test", hello: "world" };
  ...
  it('should track received messages', () => {
    expect(ws.metrics.messagesReceived).toBe(0);
      ...
      ws.ws.simulateMessage({ type: 'test', data: {} });

    expect(ws.metrics.messagesReceived).toBe(1);
  });

    it('should track connection time', () => {
      expect(ws.metrics.connectionTime).toBeGreaterThan(0);
    });

    it('should reset metrics on reconnect', async () => {
      ws.send({ test: 'data' });

      expect(ws.metrics.messagesSent).toBeGreaterThan(0);
      ...
      // Metrics should be preserved across reconnects
      expect(ws.metrics.messagesSent).toBeGreaterThan(0);
    });
  });
});
