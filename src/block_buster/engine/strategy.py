...
decision_metric = metrics.Counter(
    "trade_decisions_total", "trade decisions (sampled)", ["decision"]
)
_TRADE_SAMPLE = max(1, int(os.getenv("TRADE_DECISION_SAMPLE", "5")))
...
class Strategy:
    ...
    def evaluate(
        ...
        decision = "enter" if qty > 0 else "skip"
        self._decision_count += 1
        if self._decision_count % _TRADE_SAMPLE == 0:
            decision_metric.labels(decision=decision).inc()
        if qty <= 0:
            return None
...
class VolumeAwareStrategy:
    ...
    def should_enter(self, features: Sequence[float]) -> bool:
        ...
        self.sample_count += 1
        if self.sample_count % _TRADE_SAMPLE == 0:
            decision_metric.labels(decision="enter_check").inc()
        return score > 0.5
    ...
    def should_exit(self, features: Sequence[float]) -> bool:
        ...
        self.sample_count += 1
        if self.sample_count % _TRADE_SAMPLE == 0:
            decision_metric.labels(decision="exit_check").inc()
        return score > 0.5
